...
public class Player implements pentos.sim.Player {
    ...
    private Move handleFactory(Building request, Land land, ArrayList<Move> possibleMoves) {
        ...
        int height = 0;
        int maxNumberAdjascent = 0;
        int distanceToRoad = Integer.MAX_VALUE;
        int numAdj = 0;
        int OFFSET = 5;
        ...
        for(Move current : possibleMoves) {
            // Get coordinates of building placement (position plus local building cell coordinates).
//            boolean alreadyConnectedToRoad = false;

            //Priority - Height > Proximity to Road > Num Adj

            if(current.location.i >= height + OFFSET || chosen_final.accept == false) {
                Set<Cell> cells = getCells(current);
                Set<Cell> road = findShortestRoad(cells, land);
                if(road == null) continue; // Could not build road to placement
                int currentDistanceToRoad = road.size();

//                if(currentDistanceToRoad <= distanceToRoad || chosen_final.accept == false) {
                    // Lets find the highest scoring building placement (i.e.)
                    // a placement that touches the most desirable sides (Other
                    // buildings, parks, ponds, roads)
                    int curr_sides = 0;
                    ...
                    for (Cell x : cells) {
                        for (int i_seg = -1; i_seg <= 1; i_seg++) {
                            for (int j_seg = -1; j_seg <= 1; j_seg++) {
                                ...
                                if ( ((!land.unoccupied(curr)) && (!cells.contains(curr))) ||
...
//                                alreadyConnectedToRoad = (road_cells_on_board.contains(curr) || alreadyConnectedToRoad);
                    ...
                    if(current.location.i >= height || curr_sides >= numAdj || chosen_final.accept == false) {
                        numAdj = curr_sides;
                        distanceToRoad = currentDistanceToRoad;
                        height = current.location.i;
                        chosen_final = current;
                        chosen_final.road = road;
                    }
//                }
            }
    ...
    private Move handleResidence(Building request, Land land, ArrayList<Move> possibleMoves) {
        ...
        for(Move current : possibleMoves) {
            ...
            boolean alreadyConnectedToPondCurrent = false;
            boolean alreadyConnectedToParkCurrent = false;
                    ...
                    for (Cell x : cells) {
                        for (int i_seg = -1; i_seg <= 1; i_seg++) {
                            for (int j_seg = -1; j_seg <= 1; j_seg++) {
                                ...
                                boolean isOccupied = isborderRoad(curr_i_actual, curr_j_actual) || (
                                        !cells.contains(curr) && (
                                                !land.unoccupied(curr) || land.isField(curr) || land.isPond(curr)
                                                )
                                        );
                                if (isOccupied)
                                {
                                ...
                                // see if you are already connected to a road
//                                alreadyConnectedToRoad = (road_cells_on_board.contains(curr) || alreadyConnectedToRoad);
                                alreadyConnectedToPondCurrent = (land.isPond(curr) || alreadyConnectedToPondCurrent);
                                alreadyConnectedToParkCurrent = (land.isField(curr) || alreadyConnectedToParkCurrent);
                            }
                        }
                    }

                    if(curr_sides >= numAdj || chosen_final.accept == false) {
                        numAdj = curr_sides;
                        distanceToRoad = currentDistanceToRoad;
                        alreadyConnectedToPark = alreadyConnectedToParkCurrent;
                        alreadyConnectedToPond = alreadyConnectedToPondCurrent;
                        height = current.location.i;
                        chosen_final = current;
                        chosen_final.road = road;
                        buildParkPonds = true;
